...
function CoreValueCard({ title, description, icon, special, onClick }: CoreValue & { onClick?: () => void }) {
  if (special) {
    return (
      <motion.div
        ...
        className="p-8 rounded-2xl shadow-lg flex flex-col justify-center bg-brand-yellow items-center text-center cursor-pointer"
        onClick={onClick}
      >
  ...
  return (
    <motion.div
      whileHover={{ scale: 1.05 }}
      className="relative p-8 pt-14 rounded-[40px] shadow-lg flex flex-col border border-brand-yellow/30 text-left bg-brand-white"
    >
      <div className="absolute -top-8 left-6 w-16 h-16 flex items-center justify-center rounded-lg bg-brand-yellow shadow-lg">
    ...
    <section className="relative w-full py-20 font-mulish">
      <div className="max-w-6xl mx-auto px-6 text-center">
        ...
        <h2 className="text-3xl md:text-4xl font-bold mb-6 mt-2 text-brand-black">
          Core Values That Drive Deadlock AI
        </h2>
        <div className="flex justify-center">
          <p className="text-lg text-brand-black/70 mb-16 w-full lg:w-[45%]">
            "At Deadlock AI, our foundation is built on excellence, integrity, and a customer-first approach. We cultivate a culture of meritocracy, celebrate achievement, and champion innovation to deliver cutting-edge payment solutions that empower merchants and society alike."
          </p>
        </div>
